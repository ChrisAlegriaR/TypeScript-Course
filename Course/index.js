...
let datoPrimitivoConMasDeUnTipoDeDatoPrimitivo = 'Hola Mundo'; //* Declara una variable utilizando Union Types, permitiendo que el valor sea string, number, null o undefined, e inicializándola con un string válido.
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ^Any: Cualquier dato.
...
let tipoDatoAny2; //* Declara explícitamente una variable con tipo `any`, desactivando el sistema de tipos para ella.
...
let variableTipoArray = ['Hola', 'Mundo']; //* Declara un array tipado como string[], permitiendo únicamente cadenas de texto dentro del arreglo.
...
let variableTipoArrayNumberDesgloce = [1, 2, 3, 4, 5]; //* Declara un array tipado explícitamente como number[].
...
let objetoEspecificarClaveValorDemas = {
...
function funcionUsoType(parametroRecibira) {
...
function funcionUsoType2(parametroRecibira) {
...
function funcionUsoType3(parametroRecibira) {
...
function funcionUsoInterfaces(parametroRecibira) {
...
function funcionUsoInterfaces2(parametroRecibira) {
...
function funcionUsoInterfaces3(parametroRecibira) {
...
class PeliculaClase1 {
    nombre; //* Declaración del atributo de instancia `nombre` tipado como string.
    calificacion; //* Declaración del atributo de instancia `calificacion` tipado como number.
...
class PeliculaClase2 {
    nombre; //* Atributo opcional que puede ser string o undefined.
    calificacion = 7; //* Atributo inicializado directamente con valor por defecto.
    genero; //* Atributo obligatorio que deberá inicializarse en el constructor.
    constructor(generoRecibido) {
...
class PeliculaClase3 {
    nombre; //* Atributo opcional de instancia.
    constructor(nombreRecibido) {
        this.nombre = nombreRecibido; //* Asigna el valor recibido al atributo de instancia `nombre`.
    } //* Cierre del constructor.
    asignarNombre() {
        let nombre = 'Nemo'; //* Declaración de una variable local llamada `nombre`.
...
class ClaseParaAlta {
    nombre; //* Propiedad opcional cuyo tipo dependerá del tipo que se pase al genérico.
} //* Cierre de la clase.
...
class PasoDeDatoEnGenerico {
    numero;
    constructor(numero) {
        this.numero = numero;
    } //* Constructor que declara automáticamente `numero` como propiedad pública del tipo genérico.
    getNombre() {
        console.log(`El numero pasado es: ${this.numero}`); //* Imprime el valor almacenado en la instancia.
    } //* Cierre del método.
} //* Cierre de la clase.
let instanciaPorNumber = new PasoDeDatoEnGenerico(2); //* Instancia que usa number como tipo del genérico.
let instanciaPorString = new PasoDeDatoEnGenerico('6'); //* Instancia que usa string como tipo del genérico.
...
class PropiedadesDeParametro {
    nombre;
    edad; //* Propiedad opcional que al no indicar modificador es pública por defecto.
...
class ClaseConEncapsulamiento {
    contrasena; //* Propiedad privada que no puede ser accedida fuera de la clase.
...
class Sorteo {
    nombre;
    ticket; //* Propiedad privada tipada con el genérico.
    constructor(nombre) {
        this.nombre = nombre;
    } //* Propiedad de parámetro privada.
    setTicket(number) {
...
let sorteo = new Sorteo('Christian'); //* Instancia usando number como tipo del genérico.
